...
let handler = async (m, { conn, command }) => {
  ...
  if (!admins.includes(m.sender)) return conn.sendMessage(m.chat, { text: '❌ Solo administradores pueden usar este comando.' })

  if (!groupMetadata.participants.find(p => p.id === conn.user.id)?.admin) {
    return conn.sendMessage(m.chat, { text: '❌ El bot necesita ser admin para mutear.' })
  }

  // Obtener usuario a mutear/desmutear
  let userJid = null
  ...
  if (!userJid) return conn.sendMessage(m.chat, { text: '⚠️ Debes mencionar, citar o escribir el número del usuario.' })

  const nombre = userJid.split('@')[0]

  if (ownersJids.includes(userJid)) {
    return conn.sendMessage(m.chat, { text: `❌ No puedes mutear a un owner protegido.` })
  }
