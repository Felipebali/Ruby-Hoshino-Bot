...
// Muestra la ficha del dueño o responde con un mensaje divertido si no lo es
...
const frasesNoOwner = [
  '😼 No sos dueño, pero podés ser mi asistente de snacks.',
  '🐾 A vos te falta un poco de poder gatuno para eso.',
  '💤 No sos owner, pero igual te banco, humano promedio.',
  '🦴 Lo tuyo es admirar, no mandar 😹',
  '🎭 Solo los elegidos pueden ver sus datos... vos no 😼'
];
...
let handler = async (m, { conn }) => {
  try {
    ...
    // 🧍‍♂️ Si no es dueño → mostrar todos los dueños
    if (!ownerData[sender]) {
      ...
      // Si cita un mensaje, se usa. Si no, cita el propio mensaje
      ...
      // Luego manda la respuesta chistosa
      await conn.sendMessage(m.chat, { text: fraseNoOwner }, { quoted: quotedMsg });
    }
