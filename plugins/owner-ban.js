// 📂 plugins/propietario-ln.js - Versión mejorada con emojis y nuevos comandos
...
await conn.sendMessage(m.chat, {
  text: `${EMOJIS.success} @${userJid.split('@')[0]} fue agregado a la lista negra.\n📝 Motivo: ${reason}`,
  mentions: [userJid]
})
  ...
  try {
    ...
    if (member) {
      await conn.sendMessage(jid, {
        text: `${EMOJIS.error} @${userJid.split('@')[0]} está en la lista negra y será eliminado automáticamente.\n📝 Motivo: ${reason}`,
        mentions: [userJid]
      })
...
await conn.sendMessage(m.chat, { text: `${EMOJIS.success} @${userJid.split('@')[0]} fue eliminado de la lista negra.`, mentions: [userJid] })
...
if (!db[userJid]?.banned)
return conn.sendMessage(m.chat, { text: "${EMOJIS.success} @${userJid.split('@')[0]} no está en la lista negra.", mentions: [userJid] })

await conn.sendMessage(m.chat, {
  text: `${EMOJIS.error} @${userJid.split('@')[0]} está en la lista negra.\n📝 Motivo: ${db[userJid].banReason || 'No especificado'}`,
  mentions: [userJid]
})
...
return conn.sendMessage(m.chat, { text: "${EMOJIS.success} No hay usuarios en la lista negra." })

let list = `${EMOJIS.error} *Lista negra actual:*\n\n`
const mentions = []
...
await conn.sendMessage(m.chat, { text: "${EMOJIS.success} La lista negra ha sido vaciada." })
}
...
try {
await conn.sendMessage(m.chat, {
text: "${EMOJIS.error} @${sender.split('@')[0]} está en la lista negra y será eliminado.\n📝 Motivo: ${reason}",
mentions: [sender]
})
await new Promise(r => setTimeout(r, 2500))
...
text: "${EMOJIS.error} @${u.split('@')[0]} está en la lista negra y será eliminado automáticamente.\n📝 Motivo: ${reason}",
mentions: [u]
})
await new Promise(r => setTimeout(r, 2500))
