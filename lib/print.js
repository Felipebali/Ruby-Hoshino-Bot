...
export default async function (m, conn = { user: {} }) {
    ...
    // ✅ Nueva forma segura de calcular el tamaño
    const sizeUnitIndex = filesize > 0 ? Math.floor(Math.log(filesize) / Math.log(1000)) : 0;
    const sizeValue = filesize > 0 ? (filesize / 1000 ** sizeUnitIndex).toFixed(1) : 0;
    const sizeUnit = ['', 'K', 'M', 'G', 'T', 'P'][sizeUnitIndex] || '';
...
`.trim(),
        ...
        m ? m.exp : '?',
        user
  ? `|${user.exp}|${user.limit}|${user.level ?? 0}`
  : '|0|0|0',
